...
export default function QuantityEdit() {
  ...
  useEffect(() => {
    if (currentUser && currentUser?._id) {
      fetch(`http://localhost:4001/getCartItems/${currentUser?._id}`, {
        method: "GET",
      })
        .then((res) => res.json())
  ...
  const updateCartItem = (productId, quantity) => {
    fetch("http://localhost:4001/updateCartItem", {
      method: "PUT",
      headers: {
  ...
  const removeCartItem = (productId) => {
    fetch("http://localhost:4001/removeCartItem", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
                      ...
                      {cartItems.map((item) => (
                        ...
                        >
                          <MDBCol md="2" lg="2" xl="2">
                            <MDBCardImage
                              src={`http://localhost:4001${item.productId.thumbnail}`}
                              fluid
                              className="rounded-3"
                              alt={item.name}
                            />
