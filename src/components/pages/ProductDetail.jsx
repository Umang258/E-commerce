...
let { id } = useParams();
  useEffect(() => {
    fetch(`http://localhost:4001/product/call/${id}`, {
      method: "GET",
    })
      .then((res) => res.json())
      .then((data) => {
            ...
            <div className="product-image-container">
              ...
              <div className="thumbnail-gallery">
                {selectedCategory && (
                  <>
                    <img
                      src={`http://localhost:4001${selectedCategory.thumbnail}`}
                      alt="Main Thumbnail"
                      onClick={() =>
                        handleThumbnailClick(selectedCategory.thumbnail)
                      }
                      className={
                        selectedCategory.thumbnail === selectedImage
                          ? "active"
                          : ""
                      }
                    />
                    {selectedCategory.thumbnail1.map((image, index) => (
                      <img
                        key={index}
          ...
          <Card className="border-0">
            <Card.Body>
              ...
              <Card.Title className="mb-3">
                {categories.length > 0 && categories[0].productname}
                <br />
              </Card.Title>
              <Row className="mb-4">
                <Col>
